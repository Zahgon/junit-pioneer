 ...
 */
abstract class AbstractJsonArgumentsProvider<A extends Annotation> implements ArgumentsProvider, AnnotationConsumer<A>, CartesianParameterArgumentsProvider<Object> {
    ...
    private Stream<Node> provideNodes(ExtensionContext context) {
        ...
        PioneerPreconditions.notBlank(config, format("The configuration parameter %s must not have a blank value", CONFIG_PARAM));
        String objectMapperId = AnnotationSupport.findAnnotation(context.getRequiredTestMethod(), UseObjectMapper.class).map(UseObjectMapper::value).orElse(config);
    ...
    private static Arguments createArgumentsForMethod(Method method, Node node) {
        ...
        Object[] arguments = Arrays.stream(method.getParameters()).map(parameter -> {
            Property property = parameter.getAnnotation(Property.class);
            String name = property == null ? parameter.getName() : property.value();
            return node.getNode(name).map(value -> value.value(parameter.getParameterizedType())).orElse(null);
