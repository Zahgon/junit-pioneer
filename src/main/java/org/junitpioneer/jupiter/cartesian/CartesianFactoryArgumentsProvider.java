...
class CartesianFactoryArgumentsProvider implements CartesianMethodArgumentsProvider, AnnotationConsumer<CartesianTest.MethodFactory> {
    ...
    private static Method findMethodFactory(Method testMethod, String methodFactoryName, Object testInstance, TestInstance.Lifecycle lifecycle) {
        ...
        Method factory = PioneerUtils.findMethodCurrentOrEnclosing(declaringClass, factoryName).orElseThrow(() -> new ExtensionConfigurationException("Method `Stream<? extends Arguments> " + factoryName + "()` not found in " + declaringClass + " or any enclosing class."));
        ...
        if (!ArgumentSets.class.isAssignableFrom(factory.getReturnType()))
            throw new ExtensionConfigurationException(format("%s must return a `%s` object", method, ArgumentSets.class.getName()));
        return factory;
    }
    ...
    private static Class<?> findExplicitOrImplicitClass(Method testMethod, String methodFactoryName) {
        ...
        while (methodClass != null) {
            ...
            tryToLoadClass = tryToLoadClass.orElse(() -> ReflectionSupport.tryToLoadClass(enclosingName + "$" + className));
            methodClass = methodClass.getEnclosingClass();
        }
        return tryToLoadClass.getOrThrow(ex -> new ExtensionConfigurationException(format("Class %s not found, referenced in method %s", className, testMethod.getName()), ex));
    }
    ...
    private ArgumentSets invokeMethodFactory(Method testMethod, Method factory, Object testInstance) {
        ...
        if (count > testMethod.getParameterCount()) {
            ...
            throw new ParameterResolutionException(format("Method `%s` must register values for each parameter exactly once. Expected [%d] parameter sets, but got [%d].", factory, testMethod.getParameterCount(), count));
        }
        return argumentSets;
    }
