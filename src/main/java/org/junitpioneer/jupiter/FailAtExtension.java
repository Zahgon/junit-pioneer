...
class FailAtExtension implements ExecutionCondition {
    ...
    private LocalDate parseDate(String dateString) {
        try {
            ...
            throw new ExtensionConfigurationException("The `failAtDate` string '" + dateString + "' is not a valid ISO 8601 string.", ex);
        }
    }
    ...
    private ConditionEvaluationResult evaluateFailAtDate(ExtensionContext context, LocalDate failAtDate) {
        ...
        if (isBefore) {
            String reportEntry = format("The `date` %s is after the current date %s, so `@FailAt` did not fail the test \"%s\". It will do so when the date is reached.", failAtDateString, todayDateString, context.getUniqueId());
            context.publishReportEntry("FailAt", reportEntry);
            return enabled(reportEntry);
        } else {
            String reportEntry = format("The current date %s is after or on the `date` %s, so `@FailAt` fails the test \"%s\". Please remove the annotation.", failAtDateString, todayDateString, context.getUniqueId());
            context.publishReportEntry(FailAtExtension.class.getSimpleName(), reportEntry);
            String message = format("The current date %s is after or on the `date` %s", todayDateString, failAtDateString);
