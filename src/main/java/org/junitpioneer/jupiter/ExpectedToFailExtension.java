...
class ExpectedToFailExtension implements Extension, InvocationInterceptor {
    ...
    private static void invokeAndInvertResult(Invocation<Void> invocation, ExtensionContext extensionContext) throws Throwable {
        ...
        try {
            ...
            if (Stream.of(expectedToFail.withExceptions()).noneMatch(clazz -> clazz.isInstance(t))) {
                throw new AssertionFailedError("Test marked as temporarily 'expected to fail' failed with an unexpected exception", t);
            }
    ...
    private static ExpectedToFail getExpectedToFailAnnotation(ExtensionContext context) {
        return AnnotationSupport.findAnnotation(context.getRequiredTestMethod(), ExpectedToFail.class).orElseThrow(() -> new IllegalStateException("@ExpectedToFail is missing."));
    }
}
