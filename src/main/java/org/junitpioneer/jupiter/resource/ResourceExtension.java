...
class ResourceExtension implements ParameterResolver, InvocationInterceptor {
    ...
    NAMESPACE = ExtensionContext.Namespace.create(ResourceExtension.class);
    ...
    private Object resolveNew(New newAnnotation, ExtensionContext.Store store) {
        ...
        try {
            ...
            // @formatter:off
            String message = format("Unable to get the contents of the resource created by `%s`", resourceFactory.getClass().getTypeName());
            // @formatter:on
            throw new ParameterResolutionException(message, ex);
        }
        if (result == null) {
            // @formatter:off
            String message = format("The resource returned by [%s] was null, which is not allowed", getMethod(resource.getClass(), "get"));
            // @formatter:on
            throw new ParameterResolutionException(message);
        }
        return result;
    ...
    private Object resolveShared(Shared sharedAnnotation, Parameter[] parameters, ExtensionContext.Store scopedStore, ExtensionContext.Store rootStore) {
        ...
        try {
            ...
            __ -> ReflectionSupport.newInstance(sharedAnnotation.factory()), ResourceFactory.class);
            Resource<?> resource = scopedStore.//
            computeIfAbsent(//
            resourceKey(sharedAnnotation), //
            __ -> newResource(sharedAnnotation, resourceFactory), Resource.class);
            ...
            try {
                ...
                // @formatter:off
                String message = format("Unable to get the contents of the resource created by `%s`", sharedAnnotation.factory());
                // @formatter:on
                throw new ParameterResolutionException(message, ex);
            }
            if (result == null) {
    ...
    private Resource<?> newResource(Object newOrSharedAnnotation, ResourceFactory<?> resourceFactory) {
        ...
        try {
            ...
            //
            String //
            message = format("Unable to create a resource from `%s`", resourceFactory.getClass().getTypeName());
            throw new ParameterResolutionException(message, ex);
        }
        if (result == null) {
            // @formatter:off
            String message = format("The `Resource` instance returned by the factory method [%s] with arguments %s was null, which is not allowed", getMethod(resourceFactory.getClass(), "create", List.class), arguments);
            // @formatter:on
            throw new ParameterResolutionException(message);
        }
        return result;
    ...
    private void throwIfHasAnnotationWithSameNameButDifferentType(ExtensionContext.Store scopedStore, Shared sharedAnnotation) {
        //
        ResourceFactory<?> //
        presentResourceFactory = scopedStore.get(factoryKey(sharedAnnotation), ResourceFactory.class);
        if (presentResourceFactory == null) {
            scopedStore.put(keyOfFactoryKey(sharedAnnotation), factoryKey(sharedAnnotation));
        } else {
            //
            String //
            presentResourceFactoryName = scopedStore.get(keyOfFactoryKey(sharedAnnotation), String.class);
            if (factoryKey(sharedAnnotation).equals(presentResourceFactoryName) && !sharedAnnotation.factory().equals(presentResourceFactory.getClass())) {
                // @formatter:off
                String message = format("Two or more parameters are annotated with @Shared annotations with the name \"%s\" " + "but with different factory classes", sharedAnnotation.name());
                // @formatter:on
                throw new ParameterResolutionException(message);
            }
        }
    ...
    private void throwIfHasAnnotationWithSameNameButDifferentScope(ExtensionContext.Store rootStore, Shared sharedAnnotation) {
        ...
        if (presentSharedAnnotation == null) {
            ...
            if (presentSharedAnnotation.name().equals(sharedAnnotation.name()) && !presentSharedAnnotation.scope().equals(sharedAnnotation.scope())) {
                // @formatter:off
                String message = format("Two or more parameters are annotated with @Shared annotations with the name " + "\"%s\" but with different scopes", sharedAnnotation.name());
                // @formatter:on
                throw new ParameterResolutionException(message);
            }
        }
    ...
    private void throwIfMultipleParametersHaveExactAnnotation(Parameter[] parameters, Shared sharedAnnotation) {
        //
        long //
        parameterCount = Arrays.stream(parameters).filter(parameter -> hasAnnotation(parameter, sharedAnnotation)).count();
        if (parameterCount > 1) {
            // @formatter:off
            String message = format("A test method has %d parameters annotated with @Shared with the same factory type " + "and name; this is redundant, so it is not allowed", parameterCount);
            // @formatter:on
            throw new ParameterResolutionException(message);
        }
    }
    ...
    private boolean hasAnnotation(Parameter parameter, Shared sharedAnnotation) {
        return AnnotationSupport.findAnnotation(parameter, Shared.class).filter(shared -> shared.factory().equals(sharedAnnotation.factory())).filter(shared -> shared.name().equals(sharedAnnotation.name())).isPresent();
    }
    ...
    private Method getMethod(Class<?> clazz, String method, Class<?>... parameterTypes) {
        return ReflectionSupport.findMethod(clazz, method, parameterTypes).orElseThrow(() -> new IllegalStateException(format("There should be a `%s` method on class `%s`", method, clazz.getTypeName())));
    }
    ...
    private <T> T runSequentially(Invocation<T> invocation, Executable executable, ExtensionContext extensionContext) throws Throwable {
    ...
    private List<ReentrantLock> sortedLocksForSharedResources(Collection<Shared> sharedAnnotations, ExtensionContext extensionContext) {
        List<Shared> sortedAnnotations = sharedAnnotations.stream().sorted(comparing(Shared::name)).toList();
        //
        List<ExtensionContext.Store> //
        stores = sortedAnnotations.//
        stream().map(shared -> scopedStore(extensionContext, shared.scope())).toList();
        return IntStream.range(0, //
        sortedAnnotations.size()).mapToObj(i -> findLockForShared(sortedAnnotations.get(i), stores.get(i))).toList();
    }

    private Method testFactoryMethod(ExtensionContext extensionContext) {
        return extensionContext.getParent().orElseThrow(() -> new IllegalStateException("The parent extension context of a DynamicTest was not a @TestFactory-annotated test method")).getRequiredTestMethod();
    }
    ...
    private List<Shared> findShared(Executable executable) {
        return Arrays.stream(executable.getParameters()).map(parameter -> AnnotationSupport.findAnnotation(parameter, Shared.class)).filter(Optional::isPresent).map(Optional::get).toList();
    }
    ...
    private ReentrantLock findLockForShared(Shared shared, ExtensionContext.Store store) {
        return Optional.ofNullable(store.get(resourceLockKey(shared), ReentrantLock.class)).orElseThrow(() -> new IllegalStateException(format("There should be a shared resource for the name %s", shared.name())));
    }
