...
class DisableIfTestFailsExtension implements TestExecutionExceptionHandler, ExecutionCondition {
    ...
    private static Stream<Configuration> findConfigurations(ExtensionContext context) {
        ...
        Stream<Configuration> onParentClassConfigs = context.getParent().map(DisableIfTestFailsExtension::findConfigurations).orElse(Stream.empty());
        List<Configuration> configurations = Stream.concat(onClassConfig, onParentClassConfigs).collect(toUnmodifiableList());
    ...
    private static Stream<Configuration> createConfigurationFor(ExtensionContext context, List<DisableIfTestFails> annotations) {
        ...
        if (annotations.isEmpty())
            return Stream.empty();
        Set<Class<? extends Throwable>> onClassExceptions = annotations.stream().map(DisableIfTestFails::with).// If the exceptions array is empty, we later need to disable on all exceptions.
        // The easiest way to achieve that is by replacing the empty array with a Throwable.class
        // because all exceptions extend it.
        flatMap(exceptions -> exceptions.length == 0 ? Stream.of(Throwable.class) : Arrays.stream(exceptions)).collect(toSet());
    ...
    private static Stream<DisableIfTestFails> findAnnotationOn(Class<?> element) {
        if (element == null || element == Object.class)
            return Stream.empty();
        Stream<DisableIfTestFails> onElement = AnnotationSupport.findAnnotation(element, DisableIfTestFails.class).stream();
        Stream<DisableIfTestFails> onInterfaces = Arrays.stream(element.getInterfaces()).flatMap(DisableIfTestFailsExtension::findAnnotationOn);
