...
class DefaultTimeZoneExtension implements BeforeEachCallback, AfterEachCallback {
    ...
    private static void validateCorrectConfiguration(DefaultTimeZone annotation) {
        ...
        if (noValue == noProvider)
            throw new ExtensionConfigurationException("Either a valid time zone id or a TimeZoneProvider must be provided to " + DefaultTimeZone.class.getSimpleName());
    }
    ...
    private static TimeZone createTimeZone(Class<? extends TimeZoneProvider> providerClass) {
        try {
            ...
            throw new ExtensionConfigurationException("Could not instantiate TimeZoneProvider because of exception", exception);
        }
    }
